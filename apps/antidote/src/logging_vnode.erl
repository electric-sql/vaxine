...
%Actually: disklog(), which is not exported
-type disklog() :: term().
...
%% Returns all log entries from the given node with local log number N, such that From <= N <= To
-spec read_from_to({partition(), node()}, log_id(), log_opid(), log_opid()) ->
    {ok, [{non_neg_integer(), log_record()}]} | {error, term()}.
...
get_all(IndexNode, LogId, Continuation, PrevOps) ->
    riak_core_vnode_master:sync_command(
        IndexNode,
        {get_all, LogId, Continuation, PrevOps},
        ?LOGGING_MASTER,
        infinity
    ).
...
request_bucket_op_id(IndexNode, DCID, Bucket, Partition) ->
    riak_core_vnode_master:sync_command(
        IndexNode,
        {get_op_id, DCID, Bucket, Partition},
        ?LOGGING_MASTER,
        infinity
    ).
...
handle_command(
    {get_op_id, DCID, Bucket, Partition}, _Sender, State = #state{op_id_table = OpIdTable}
) ->
...
handle_command(
    {start_timer, Sender},
    _,
    State = #state{partition = Partition, op_id_table = OpIdTable, recovered_vector = MaxVector}
) ->
    ...
    IsReady =
        try
            ...
            _:Reason ->
                ?LOG_DEBUG(
                    "Error updating inter_dc_log_sender_vnode last sent log id: ~w, will retry", [
                        Reason
                    ]
                ),
                false
...
handle_command(
    {read_from_to, LogId, From, To},
    _Sender,
    #state{partition = Partition, logs_map = Map, last_read_map = LastReadMap} = State
) ->
    case get_log_from_map(Map, Partition, LogId) of
        {ok, Log} ->
            ok = disk_log:sync(Log),
            ...
            Node = {node(), dc_utilities:get_my_dc_id()},
            case read_from_to_internal(Log, Node, Continuation, LastOpId, [], To) of
                {error, Reason} ->
                    {reply, {error, Reason}, State};
                {NewContinuation, NewLastOpId, Ops} ->
                    FilteredOps = filter_operations(Ops, From, To),
                    {reply, {ok, FilteredOps}, State#state{
                        last_read_map = dict:store(
                            LogId, {NewLastOpId, NewContinuation}, LastReadMap
                        )
                    }}
            end;
        {error, Reason} ->
...
handle_command(
    {append, LogId, LogOperation, Sync},
    _Sender,
    #state{
        logs_map = Map,
        op_id_table = OpIdTable,
...
%%                      {reply, {ok, op_number()} #state{}} | {reply, error(), #state{}}.
handle_command(
    {append_group, LogId, LogRecordList, _IsLocal = false, Sync},
    _Sender,
    #state{
        logs_map = Map,
        op_id_table = OpIdTable,
                    ...
                    {reply,
                        #snapshot_get_response{
                            ...
                            materialized_snapshot = #materialized_snapshot{
                                last_op_id = 0, value = clocksi_materializer:new(Type)
                            },
...
handle_command(
    {get_all, LogId, Continuation, Ops},
    _Sender,
    #state{logs_map = Map, partition = Partition} = State
) ->
    case get_log_from_map(Map, Partition, LogId) of
...
-spec read_internal(log_id(), disk_log:continuation() | start | eof | error, [
...
%% Reads from the disk-log
%% Log: the log to read from
%% Node: the current node
%% Continuation: A continuation of the disk log
%% LastOpId: All entries in the continuation must be >= this local op id
%% Ops: Accumulator of the operations read so far
%% To: The maximum local op id to include
%% Returns: 3-tuple
%% 1. A continuation with 2. the maximum op id read before the continuation
%% 3. the log records from node that are <= To
%% The returned continuation is <= To + 1, so that it can be used for continuing to read after To.
-spec read_from_to_internal(
    log_id(),
    {node(), dcid()},
...
read_from_to_internal(Log, Node, Continuation, LastOpId, Ops, To) ->
    ...
    case Chunk of
        {error, Reason} ->
            {error, Reason};
        {NewContinuation, NewOps} ->
            read_from_to_internal2(
                Log, Node, NewContinuation, Continuation, NewOps, LastOpId, Ops, To
            );
        {NewContinuation, NewOps, _} ->
            read_from_to_internal2(
                Log, Node, NewContinuation, Continuation, NewOps, LastOpId, Ops, To
            );
        eof ->
            {Continuation, LastOpId, Ops}
    end.

-spec read_from_to_internal2(
    log_id(),
    {node(), dcid()},
    disk_log:continuation(),
    disk_log:continuation(),
...
read_from_to_internal2(Log, Node, NewContinuation, LastContinuation, NewOps1, LastLOpId, Ops, To) ->
    ...
    case NewOps of
        [] ->
            read_from_to_internal(Log, Node, NewContinuation, LastLOpId, Ops, To);
        [{_, FirstLogRecord} | _] ->
            FirstLOpId = FirstLogRecord#log_record.op_number#op_number.local,
            {_, LastLogRecord} = lists:last(NewOps),
            NewLastLOpId = LastLogRecord#log_record.op_number#op_number.local,
            % this could be optimized, since we know that operations are ordered
            OpsInRange = filter_operations(NewOps, LastLOpId, To),
            if
                NewLastLOpId < To ->
                    read_from_to_internal(
                        Log, Node, NewContinuation, NewLastLOpId + 1, Ops ++ OpsInRange, To
                    );
                true ->
                    {LastContinuation, FirstLOpId, Ops ++ OpsInRange}
            end
    end.

%% Only return those operations that originate from the given node
-spec filter_local_node({node(), dcid()}, [{non_neg_integer(), log_record()}]) ->
    [{non_neg_integer(), log_record()}].
filter_local_node(Node, Ops) ->
    [{N, Rec} || {N, Rec} <- Ops, (Rec#log_record.op_number)#op_number.node == Node].

%% Only return those operations where the local operation id is in the interval [Min, Max]
-spec filter_operations([{non_neg_integer(), log_record()}], log_opid(), log_opid()) ->
    [{non_neg_integer(), log_record()}].
...
-spec reverse_and_add_op_id([clocksi_payload()], non_neg_integer(), [
    {non_neg_integer(), clocksi_payload()}
]) ->
    [{non_neg_integer(), clocksi_payload()}].
...
-spec get_last_op_from_log(log_id(), disk_log:continuation() | start, cache_id(), vectorclock()) ->
    {eof, vectorclock()} | {error, term()}.
...
%% After appending an operation to the log, increment the op id
-spec update_ets_op_id({log_id(), dcid()} | {log_id(), bucket(), dcid()}, op_number(), cache_id()) ->
    true.
...
-spec get_ops_from_log(
    ...
    | {eof, dict:dict(key(), [{non_neg_integer(), clocksi_payload()}])}.
get_ops_from_log(
    Log, Key, Continuation, MinSnapshotTime, MaxSnapshotTime, Ops, CommittedOpsDict, LoadAll
) ->
    case disk_log:chunk(Log, Continuation) of
        eof ->
        ...
        {error, Reason} ->
            {error, Reason};
        {NewContinuation, NewTerms} ->
            {NewOps, NewCommittedOps} = filter_terms_for_key(
                NewTerms, Key, MinSnapshotTime, MaxSnapshotTime, Ops, CommittedOpsDict
            ),
            case LoadAll of
                load_all ->
                    get_ops_from_log(
...
-spec finish_op_load(dict:dict(key(), [clocksi_payload()])) ->
    dict:dict(key(), [{non_neg_integer(), clocksi_payload()}]).
finish_op_load(CommittedOpsDict) ->
    dict:fold(
        fun(Key1, CommittedOps, Acc) ->
...
filter_terms_for_key(
    [{_, LogRecord} | T], Key, MinSnapshotTime, MaxSnapshotTime, Ops, CommittedOpsDict
) ->
...
handle_commit(TxId, OpPayload, T, Key, MinSnapshotTime, MaxSnapshotTime, Ops, CommittedOpsDict) ->
    #commit_log_payload{commit_time = {DcId, TxCommitTime}, snapshot_time = SnapshotTime} =
        OpPayload,
    case dict:find(TxId, Ops) of
        {ok, OpsList} ->
            NewCommittedOpsDict =
                lists:foldl(
                    fun(#update_log_payload{key = KeyInternal, type = Type, op = Op}, Acc) ->
                        case
                            (check_min_time(SnapshotTime, MinSnapshotTime) andalso
...
handle_handoff_command(
    #riak_core_fold_req_v2{foldfun = FoldFun, acc0 = OldHandoffState},
    _Sender,
    #state{logs_map = Map, partition = Partition} = State
) ->
...
handle_handoff_data(
    Data, #state{partition = Partition, logs_map = Map, enable_log_to_disk = EnableLog} = State
) ->
...
-spec open_logs(string(), [preflist()], dict:dict(log_id(), disklog()), cache_id(), vectorclock()) ->
    {dict:dict(log_id(), disklog()), vectorclock()} | {error, reason()}.
...
open_logs(LogFile, [Next | Rest], Map, ClockTable, MaxVector) ->
    ...
    case disk_log:open([{name, LogPath}]) of
        {ok, Log} ->
            ...
            ?LOG_DEBUG("Opened log ~p, last op ids are ~p, max vector is ~p", [
                Log, get_op_numbers(ClockTable), vectorclock:to_list(NewMaxVector)
            ]),
            Map2 = dict:store(PartitionList, Log, Map),
            %%TODO Why MaxVector here? Maybe NewMaxVector
            open_logs(LogFile, Rest, Map2, ClockTable, MaxVector);
        {repaired, Log, _, _} ->
            {eof, NewMaxVector} = get_last_op_from_log(Log, start, ClockTable, MaxVector),
            ?LOG_DEBUG("Repaired log ~p, last op ids are ~p, max vector is ~p", [
                Log, get_op_numbers(ClockTable), vectorclock:to_list(NewMaxVector)
            ]),
            Map2 = dict:store(PartitionList, Log, Map),
...
%%
-spec get_log_from_map(
    dict:dict(log_id(), disklog() | {log_opid(), disk_log:continuation()}), partition(), log_id()
) ->
...
-spec get_op_number(cache_id(), {log_id(), dcid()} | {log_id(), bucket(), dcid()}) ->
    not_found | {ok, op_number()}.
...
-spec get_op_numbers(cache_id()) ->
    [{log_id(), dcid(), op_number()} | {log_id(), bucket(), dcid(), op_number()}].
get_op_numbers(ClockTable) ->
    ets:tab2list(ClockTable).

-spec insert_op_number(cache_id(), {log_id(), dcid()} | {log_id(), bucket(), dcid()}, op_number()) ->
    true.
insert_op_number(ClockTable, Key, NewOp) ->
    ets:insert(ClockTable, {Key, NewOp}).

data_dir() ->
    case application:get_env(antidote, data_dir) of
        {ok, DataDir} -> DataDir;
        undefined -> "data"
    end.
...
init_log(LogId, Partition) ->
    ...
    LogFile = integer_to_list(Partition),
    OpIdTable = create_op_id_table(),
    File = filename:join(DataDir, integer_to_list(Partition) ++ "--0.LOG"),
    ...
    {Map, MaxVector} = open_logs(
        LogFile, [[{LogId, node}]], dict:new(), OpIdTable, vectorclock:new()
    ),
    #state{
        partition = Partition,
        logs_map = Map,
...
log_cleanup(State) ->
    ...
    file:delete(filename:join(DataDir, File)),
    file:del_dir(DataDir).

append_log_record(_, _, _, 0, State) ->
    {[], State};
append_log_record(
    LogId,
    Node,
    ...
    % TODO only increment local for same nodes
    NewOpId = #op_number{node = Node, local = LocalLogId, global = Global + 1},
    true = update_ets_op_id({LogId, MyDCID}, NewOpId, OpIdTable),
...
read_internal_test() ->
    test_with_log(0, fun(State) ->
        LogId = [0],
        {Ops, NewState} = append_log_record(LogId, node1(), 1, 20, State),
        {eof, ReadOps} = read_all_records(LogId, NewState),
        ?assertEqual(Ops, ReadOps)
    end).

read_from_to_internal_test() ->
    test_with_log(1, fun(State) ->
        LogId = [0],
        {_, NewState} = append_log_record(LogId, node1(), 0, 2000, State),
        {Ops, NewState2} = append_log_record(LogId, node2(), 0, 2000, NewState),
        {_, NewState3} = append_log_record(LogId, node3(), 0, 2000, NewState2),
        #state{logs_map = Map, partition = Partition} = NewState3,
        {ok, Log} = get_log_from_map(Map, Partition, LogId),
        ok = disk_log:sync(Log),
        {_, _, ReadOps} = read_from_to_internal(Log, node2(), start, 0, [], 4000),
        ?assertEqual(Ops, ReadOps)
    end).
...
read_from_to_internal2_test() ->
    test_with_log(2, fun(State) ->
        ...
        {Ops, NewState2} = append_log_record(LogId, node2(), 1, 4000, NewState),
        {_, NewState3} = append_log_record(LogId, node3(), 1, 2000, NewState2),
        #state{logs_map = Map, partition = Partition} = NewState3,
        {ok, Log} = get_log_from_map(Map, Partition, LogId),
        ok = disk_log:sync(Log),
        {Continuation, LastOpId, ReadOps1} = read_from_to_internal(
            Log, node2(), start, 0, [], 2000
        ),
        ?assertEqual(filter_operations(Ops, 1, 1), [hd(ReadOps1)]),
        ?assertEqual(filter_operations(Ops, 2000, 2000), [lists:last(ReadOps1)]),
        ?assertEqual(filter_operations(Ops, 1, 2000), ReadOps1),
        {_, _, ReadOps2} = read_from_to_internal(Log, node2(), Continuation, LastOpId, [], 4000),
...
read_from_to_internal3_test() ->
    test_with_log(3, fun(State) ->
        ...
        {Ops, NewState2} = append_log_record(LogId, node2(), 1, 10, NewState),
        {_, NewState3} = append_log_record(LogId, node3(), 1, 2, NewState2),
        #state{logs_map = Map, partition = Partition} = NewState3,
        {ok, Log} = get_log_from_map(Map, Partition, LogId),
        ok = disk_log:sync(Log),
        {_, _, ReadOps1} = read_from_to_internal(Log, node2(), start, 1, [], 3),
        ?assertEqual(filter_operations(Ops, 1, 3), ReadOps1),
        {_, _, ReadOps2} = read_from_to_internal(Log, node2(), start, 6, [], 8),
        ?assertEqual(filter_operations(Ops, 6, 8), ReadOps2)
    end).
