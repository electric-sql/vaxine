...
-record(state, {
    ...
    %% DCID -> queue()
    queues :: dict:dict(dcid(), queue:queue()),
...
set_dependency_clock(Partition, Vector) ->
    dc_utilities:call_local_vnode_sync(
        Partition, inter_dc_dep_vnode_master, {set_dependency_clock, Vector}
    ).
...
process_queue(DCID, {State, Acc}) ->
    ...
    case queue:peek(Queue) of
        ...
        {value, Txn} ->
            ...
            case Success of
                false -> {NewState, Acc};
                %% remove the just-applied txn and retry
                true -> process_queue(DCID, {pop_txn(NewState, DCID), Acc + 1})
            end
    end.
...
try_store(
    State,
    Txn = #interdc_txn{dcid = DCID, partition = Partition, timestamp = Timestamp, log_records = Ops}
) ->
...
get_partition_clock(State) ->
    ...
    vectorclock:set(
        dc_utilities:get_my_dc_id(), dc_utilities:now_microsec(), State#state.vectorclock
    ).
...
updates_to_clocksi_payloads(
    Txn = #interdc_txn{dcid = DCID, timestamp = CommitTime, snapshot = SnapshotTime}
) ->
    lists:map(
        fun(#log_record{log_operation = LogRecord}) ->
            #update_log_payload{key = Key, type = Type, op = Op} =
                LogRecord#log_operation.log_payload,
            #clocksi_payload{
                key = Key,
                type = Type,
                op_param = Op,
        ...
        inter_dc_txn:ops_by_type(Txn, update)
    ).
