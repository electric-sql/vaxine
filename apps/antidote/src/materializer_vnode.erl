...
-spec read(
    key(), type(), snapshot_time(), txid(), clocksi_readitem:read_property_list(), partition_id()
...
handle_command({check_ready}, _Sender, State = #state{partition = Partition, is_ready = IsReady}) ->
    Result =
        case has_ops_cache(Partition) of
            false ->
                false;
            true ->
    ...
    IsReady =
        try
            case load_from_log_to_tables(Partition, State) of
                ok ->
                    ?LOG_DEBUG("Finished loading from log to materializer on partition ~w", [
                        Partition
                    ]),
                    true;
                {error, not_ready} ->
...
handle_handoff_command(
    #riak_core_fold_req_v2{foldfun = Fun, acc0 = Acc0},
    _Sender,
...
loop_until_loaded(Node, LogId, Continuation, Ops, State) ->
    case logging_vnode:get_all(Node, LogId, Continuation, Ops) of
        {error, Reason} ->
            ?LOG_ERROR("Could not load all entries from log ~p and node ~p: ~p", [
                LogId, Node, Reason
            ]),
            {error, Reason};
...
-spec internal_store_ss(key(), materialized_snapshot(), snapshot_time(), boolean(), state()) ->
    boolean().
internal_store_ss(
    Key,
    Snapshot = #materialized_snapshot{last_op_id = NewOpId},
    CommitTime,
    ShouldGc,
    ...
    ShouldInsert =
        case vector_orddict:size(SnapshotDict) > 0 of
            true ->
                {_Vector, #materialized_snapshot{last_op_id = OldOpId}} = vector_orddict:first(
                    SnapshotDict
                ),
...
-spec internal_validate_or_read(
    key(),
    type(),
    object_token(),
    snapshot_time(),
...
%%
-spec get_from_snapshot_cache(txid() | ignore, key(), type(), snapshot_time(), state()) ->
    snapshot_get_response().

get_from_snapshot_cache(
    TxId,
    Key,
    Type,
    MinSnaphsotTime,
...
%%
-spec store_snapshot(
    txid() | ignore, key(), materialized_snapshot(), snapshot_time(), boolean(), state()
) -> ok.
...
-spec update_snapshot_from_cache(
    {{snapshot_time() | ignore, materialized_snapshot()}, boolean()}, key(), cache_id()
) ->
    snapshot_get_response().
...
materialize_snapshot(
    ...
    ShouldGC,
    State,
    SnapshotResponse = #snapshot_get_response{
        ...
        {ok, MaterializedSnapshot, NewLastOp, SnapshotTime, WasUpdated, OpsAdded} ->
            ...
            case SnapshotTime of
                ...
                _ ->
                    ...
                    ok =
                        case ShouldRefreshCache orelse ShouldGC of
                            false ->
                                ok;
                            true ->
...
-spec snapshot_insert_gc(
    key(),
    vector_orddict:vector_orddict(),
    boolean(),
    state()
) -> true.
snapshot_insert_gc(Key, SnapshotDict, ShouldGc, #state{
    snapshot_cache = SnapshotCache, ops_cache = OpsCache
}) ->
    ...
    case ((vector_orddict:size(SnapshotDict)) >= ?SNAPSHOT_THRESHOLD) orelse ShouldGc of
        true ->
            ...
            NewListLen =
                case NewLength > ListLen - ?RESIZE_THRESHOLD of
                    true ->
                        ListLen * 2;
                    false ->
            ...
            NewTuple = erlang:make_tuple(?FIRST_OP + NewListLen, 0, [
...
prune_ops({Len, OpsTuple}, Threshold) ->
    ...
    %% one for including
    {NewSize, NewOps} = check_filter(
        fun({_OpId, Op}) ->
            OpCommitTime = Op#clocksi_payload.commit_time,
            (materializer:belongs_to_snapshot_op(
                Threshold, OpCommitTime, Op#clocksi_payload.snapshot_time
            ))
        end,
        ?FIRST_OP,
        ?FIRST_OP + Len,
...
op_insert_gc(Key, DownstreamOp, State = #state{ops_cache = OpsCache}) ->
    ...
    case has_ops_for_key(OpsCache, Key) of
        false ->
            insert_ops_cache_tuple(
                OpsCache,
                erlang:make_tuple(?FIRST_OP + ?OPS_THRESHOLD, 0, [
    ...
    case (Length >= ListLen) orelse ((NewId rem ?OPS_THRESHOLD) == 0) of
        true ->
            ...
            %% Insert the new op
            true = update_ops_element(OpsCache, Key, [
                {NewLength + ?FIRST_OP, {NewId, DownstreamOp}}, {2, {NewLength + 1, NewListLen}}
            ]);
        false ->
            true = update_ops_element(OpsCache, Key, [
...
-spec get_ops_from_cache(cache_id(), key()) ->
    not_found
    | {ok, {
        key(), non_neg_integer(), non_neg_integer(), non_neg_integer(), [op_and_id()] | tuple()
    }}.
...
gc_test() ->
    ...
    lists:map(
        fun(N) ->
            {ok, Res} = internal_read(
                Key, Type, vectorclock:from_list([{DC1, N * 10 + 2}]), ignore, [], false, State
            ),
            ?assertEqual(N, Type:value(Res)),
            op_insert_gc(Key, generate_payload(N * 10, N * 10 + 1, Res, a), State)
        end,
        lists:seq(0, ?SNAPSHOT_THRESHOLD)
    ),
    ...
    %% Trigger the clean
    {ok, Res10} = internal_read(
        Key, Type, vectorclock:from_list([{DC1, 102}]), ignore, [], true, State
    ),
    ...
    {ok, Res13} = internal_read(
        Key, Type, vectorclock:from_list([{DC1, 142}]), ignore, [], true, State
    ),
...
large_list_test() ->
    ...
    {ok, Res0} = internal_read(
        Key, Type, vectorclock:from_list([{DC1, 2}]), ignore, [], false, State
    ),
    ...
    {ok, Res1000} = internal_read(
        Key, Type, vectorclock:from_list([{DC1, 2000}]), ignore, [], false, State
    ),
...
seq_write_test() ->
    ...
    {ok, Res1} = internal_read(
        Key, Type, vectorclock:from_list([{DC1, 16}]), ignore, [], false, State
    ),
    ...
    {ok, Res2} = internal_read(
        Key, Type, vectorclock:from_list([{DC1, 21}]), ignore, [], false, State
    ),
    ?assertEqual(2, Type:value(Res2)),

    %% Read old version
    {ok, ReadOld} = internal_read(
        Key, Type, vectorclock:from_list([{DC1, 16}]), ignore, [], false, State
    ),
...
multipledc_write_test() ->
    ...
    op_insert_gc(Key, DownstreamOp1, State),
    {ok, Res1} = internal_read(
        Key, Type, vectorclock:from_list([{DC1, 16}, {DC2, 0}]), ignore, [], false, State
    ),
    ?assertEqual(1, Type:value(Res1)),
    ...
    op_insert_gc(Key, DownstreamOp2, State),
    {ok, Res2} = internal_read(
        Key, Type, vectorclock:from_list([{DC1, 16}, {DC2, 21}]), ignore, [], false, State
    ),
    ?assertEqual(2, Type:value(Res2)),

    %% Read old version
    {ok, ReadOld} = internal_read(
        Key, Type, vectorclock:from_list([{DC1, 15}, {DC2, 15}]), ignore, [], false, State
    ),
    ?assertEqual(1, Type:value(ReadOld)).
...
concurrent_write_test() ->
    ...
    op_insert_gc(Key, DownstreamOp1, State),
    {ok, Res1} = internal_read(
        Key, Type, vectorclock:from_list([{DC2, 1}, {DC1, 0}]), ignore, [], false, State
    ),
    ?assertEqual(1, Type:value(Res1)),

    %% Another concurrent increment in other DC
    {ok, Op2} = Type:downstream({increment, 1}, S1),
    DownstreamOp2 = #clocksi_payload{
        key = Key,
        type = Type,
        op_param = Op2,
        snapshot_time = vectorclock:from_list([{DC1, 0}, {DC2, 0}]),
    ...
    {ok, ReadDC1} = internal_read(
        Key, Type, vectorclock:from_list([{DC1, 1}, {DC2, 0}]), ignore, [], false, State
    ),
    ?assertEqual(1, Type:value(ReadDC1)),
    {ok, ReadDC2} = internal_read(
        Key, Type, vectorclock:from_list([{DC1, 0}, {DC2, 1}]), ignore, [], false, State
    ),
    ?assertEqual(1, Type:value(ReadDC2)),

    %% Read snapshot including both increments
    {ok, Res2} = internal_read(
        Key, Type, vectorclock:from_list([{DC2, 1}, {DC1, 1}]), ignore, [], false, State
    ),
    ?assertEqual(2, Type:value(Res2)).
